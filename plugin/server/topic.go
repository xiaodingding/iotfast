...
type Topic struct {
	mu   sync.RWMutex // a mutex for locking the whole Topic.
...
func (x *Topic) RetainMessage(msg MessagePacket) int64 {
	...
	var r int64 = 0
	if len(n.Message.Payload) > 0 && n.Message.Retain == true {
		r = -1
	}
...
func (x *Topic) unpoperate(filter string, client string, message bool) bool {
	...
	for e.Parent != nil {
		...
		if end {
			...
			if message {
				e.Message = MessagePacket{}
			}
			end = false
		}

		// If this leaf is empty, note it as orphaned.
		orphaned = len(e.Clients) == 0 && len(e.Leaves) == 0 && !e.Message.Retain
...
func (x *Topic) poperate(topic string) *Leaf {
	var d int
	var particle string
	var hasNext = true
	n := x.Root
...
func (x *Topic) Messages(filter string) []MessagePacket {
	...
	x.mu.RLock()
	defer x.mu.RUnlock()
	return x.Root.scanMessages(filter, 0, make([]MessagePacket, 0, 32))
}
...
func (l *Leaf) scanMessages(filter string, d int, messages []MessagePacket) []MessagePacket {
