...
import { clearLog, deleteLog, logList } from '/@/api/system/monitor/loginLog';
...
export default defineComponent({
  ...
  setup() {
    const queryRef = ref();
    const {admin_login_status} = useDict('admin_login_status')
    ...
    const statusFormat = (row:TableDataRow) => {
      return selectDictLabel(unref(admin_login_status), row.status);
    };
    return {
      queryRef,
      onRowDel,
