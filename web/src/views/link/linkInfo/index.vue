<template>
  <div class="app-container">
    ...
    <div class="linkInfo-search mb15">
    <el-form :inline="true" size="default" label-width="68px">    
      ...
      <el-form-item label="连接标记" prop="mark">
        <el-input
   ...
   <el-table v-loading="loading" :data="linkInfoList" @selection-change="handleSelectionChange" style="width: 100%">
  <el-table-column type="selection" width="55" align="center" />  
  <el-table-column label="主键" align="center" prop="id" />  
  <el-table-column label="连接名称" align="center" prop="name" />  
  <el-table-column label="连接标记" align="center" prop="mark" />  
    ...
    <!-- 添加或修改连接信息管理对话框 -->
     <el-dialog :title="title"  v-model="open" width="769px">
        ...
        <el-form :model="ruleForm" ref="formRef" :rules="rules" size="default" label-width="120px">            
            ...
            <el-form-item label="连接标记" prop="mark">
                <el-input v-model="ruleForm.mark" placeholder="请输入连接标记" />
            </el-form-item>            
        </el-form>
        <template #footer>
...
import {
    ...
    addLinkInfo,
    updateLinkInfo,    
} from "/@/api/link/linkInfo";
interface LinkInfoDataState {
  ids:number[];
  loading:boolean;
// 选中表数组
// tableNames: any[];
...
export default defineComponent({
    ...
    setup() {
      ...
      const state = reactive<LinkInfoDataState>( {
      // 遮罩层
      loading: true,
      // 选中数组
      ids: [],
    ...
    const handleGetList = () =>{
      state.loading = true;
      listLinkInfo(state.queryParams).then((res:any)=>{
        state.linkInfoList = res.data.list;
      ...
      proxy.$refs.formRef.validate((valid: boolean) => {
        if (valid) {
          if (state.ruleForm.id != undefined) {
            updateLinkInfo(state.ruleForm).then((res:any)=>{
              if (res.code === 0) {
            ...
            addLinkInfo(state.ruleForm).then((res:any)=>{
              if (res.code === 0) {
    ...
    const handleDelete = (row:any)=>{
      ...
      ElMessageBox.confirm('是否确认删除连接信息管理编号为"' + ids + '"的数据项?', "警告", {
