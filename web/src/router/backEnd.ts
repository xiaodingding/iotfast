...
import { formatFlatteningRoutes, formatTwoStageRoutes, router, setAddIndexRoute, setIndexFilterMenuAndCacheTagsViewRoutes } from '/@/router/index';
...
export async function initBackEndControlRoutes() {
	...
	{
		console.log("session not find token!");
		return false;
	}
	// 触发初始化用户信息 pinia
	//useUserInfo().setUserInfos();

	//useUserInfo().setBackUserInfos(null);
	//useUserInfo().setPermissions([]);

	useUserInfo(pinia).updateUserInfos();
	...
	//console.log("back end dynamicRoutes", dynamicRoutes);
	
	await setAddRoute();

	// 添加动态路由
	await setAddIndexRoute();
	// 设置路由到 vuex routesList 中（已处理成多级嵌套路由）及缓存多级嵌套数组处理后的一维数组
	await setIndexFilterMenuAndCacheTagsViewRoutes();
}
...
export function getBackEndControlRoutes() {
	...
	*/
	return getUserMenus().then((res:any)=>{
		...
		useUserInfo().setPermissions(res.data.permissions);
