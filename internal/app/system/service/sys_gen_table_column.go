...
type genTableColumnImpl struct {
...
var genTableColumnService = genTableColumnImpl{
...
func (s *genTableColumnImpl) SelectDbTableColumnsByName(ctx context.Context, tableName string) ([]*entity.SysGenTableColumn, error) {
	db := g.DB()
	var res []*entity.SysGenTableColumn
...
func (s *genTableColumnImpl) InitColumnField(column *entity.SysGenTableColumn, table *entity.SysGenTable) {
...
// GetDbType 获取数据库类型字段
func (s *genTableColumnImpl) GetDbType(columnType string) string {
...
func (s *genTableColumnImpl) IsExistInArray(value string, array []string) bool {
	for _, v := range array {
		if v == value {
			return true
		}
...
func (s *genTableColumnImpl) GetColumnLength(columnType string) int {
...
func (s *genTableColumnImpl) IsStringObject(dataType string) bool {
	return s.IsExistInArray(dataType, s.ColumnTypeStr)
}

// IsTimeObject 判断是否是数据库时间类型
func (s *genTableColumnImpl) IsTimeObject(dataType string) bool {
	return s.IsExistInArray(dataType, s.ColumnTypeTime)
}

// IsNumberObject 是否数字类型
func (s *genTableColumnImpl) IsNumberObject(dataType string) bool {
	return s.IsExistInArray(dataType, s.ColumnTypeNumber)
}

// IsNotEdit 是否不可编辑
func (s *genTableColumnImpl) IsNotEdit(name string) bool {
	return s.IsExistInArray(name, s.ColumnNameNotEdit)
}

// IsNotList 不在列表显示
func (s *genTableColumnImpl) IsNotList(name string) bool {
	return s.IsExistInArray(name, s.ColumnNameNotList)
}

// IsNotQuery 不可用于查询
func (s *genTableColumnImpl) IsNotQuery(name string) bool {
	return s.IsExistInArray(name, s.ColumnNameNotQuery)
}

// CheckNameColumn 检查字段名后4位是否是name
func (s *genTableColumnImpl) CheckNameColumn(columnName string) bool {
	if len(columnName) >= 4 {
		end := len(columnName)
		start := end - 4
		if start <= 0 {
...
func (s *genTableColumnImpl) CheckStatusColumn(columnName string) bool {
	if len(columnName) >= 6 {
		end := len(columnName)
		start := end - 6
...
func (s *genTableColumnImpl) CheckTypeColumn(columnName string) bool {
	if len(columnName) >= 4 {
		end := len(columnName)
		start := end - 4
...
func (s *genTableColumnImpl) CheckSexColumn(columnName string) bool {
	if len(columnName) >= 3 {
		end := len(columnName)
		start := end - 3
		if start <= 0 {
...
func (s *genTableColumnImpl) SelectGenTableColumnListByTableId(ctx context.Context, tableId int64) (list []*entity.SysGenTableColumn, err error) {
	err = dao.SysGenTableColumn.Ctx(ctx).Where(dao.SysGenTableColumn.Columns().TableId, tableId).
		Order(dao.SysGenTableColumn.Columns().SortOrderQuery + " asc, " + dao.SysGenTableColumn.Columns().ColumnId + " asc").Scan(&list)
	if err != nil {
		g.Log().Error(ctx, err)
...
func (s *genTableColumnImpl) GetAllTableColumns(ctx context.Context) (list []*model.SysGenTableColumn, err error) {
	err = dao.SysGenTableColumn.Ctx(ctx).Order(dao.SysGenTableColumn.Columns().SortOrderQuery + " asc, " + dao.SysGenTableColumn.Columns().ColumnId + " asc").Scan(&list)
	if err != nil {
		g.Log().Error(ctx, err)
...
func (s *genTableColumnImpl) SetPkColumn(table *sysModel.SysGenTableEntityExtend, columns []*entity.SysGenTableColumn) {
